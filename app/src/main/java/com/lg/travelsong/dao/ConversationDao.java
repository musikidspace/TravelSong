...
public class ConversationDao {
    ...
    public Conversation queryOne(String usercode) {
        ...
        //调用getReadableDatabase方法,来初始化数据库的创建或读取
        SQLiteDatabase db = helper.getReadableDatabase();
        String sql = "select userhead, usercode, nickname, username, lasttime, lastmsg, hasdelete, istop from conversation where usercode = ?";
        ...
        if (cursor != null && cursor.getCount() > 0) {//判断cursor中是否存在数据
            ...
            while (cursor.moveToNext()) {//条件，游标能否定位到下一行
                ...
                conversation.userhead = cursor.getString(0);
                conversation.usercode = cursor.getString(1);
                conversation.nickname = cursor.getString(2);
                conversation.username = cursor.getString(3);
                conversation.lasttime = cursor.getInt(4);
                conversation.lastmsg = cursor.getString(5);
                conversation.hasdelete = cursor.getInt(6);
                conversation.istop = cursor.getInt(7);
            }
            cursor.close();//关闭结果集
        }
        //关闭数据库对象
    ...
    public List<Conversation> query(String sql) {
        ...
        if (cursor != null && cursor.getCount() > 0) {//判断cursor中是否存在数据
            ...
            while (cursor.moveToNext()) {//条件，游标能否定位到下一行
                ...
                conversation.hasdelete = cursor.getInt(cursor.getColumnIndex("hasdelete"));
